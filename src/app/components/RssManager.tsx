...
import { Search, Rss, Plus, Loader2, ExternalLink, RefreshCw, Calendar, Info, X, CheckCircle2 } from "lucide-react";
...
export default function RssManager({ theme }: RssManagerProps) {
  ...
  // Processing Progress UI Component
  const ProcessingProgressUI = () => {
    const { isProcessing, currentStep, progress, totalItems, processedItems, currentItemTitle } = processingStatus;
        ...
        <div className="bg-gray-900 rounded-xl shadow-2xl max-w-md w-full p-6 border border-gray-800">
          ...
          <div className="mb-5">
            <div className="flex justify-between mb-2">
              <span className="text-blue-400 animate-pulse font-medium">{currentStep}</span>
            </div>
          </div>
                ...
                {currentStep === "Completed" ? (
                  ...
                  <>
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-gray-300 truncate">{currentItemTitle || "Processing..."}</span>
                    </div>
                    <div className="text-xs text-gray-500">
                      {currentStep === "Extracting Blog Posts" ? "Parsing and humanizing content" :
                        currentStep === "Saving to Database" ? "Writing processed data to database" :
                          "Working..."}
                    </div>
                  </>
                )}
          ...
          {currentStep === "Completed" ? (
            <div className="bg-green-900/30 border border-green-800 text-green-400 px-4 py-3 rounded-lg text-sm flex items-center">
              <CheckCircle2 className="h-5 w-5 mr-2" />
              <span>RSS feed successfully processed and added to your collection</span>
            </div>
          ) : (
    ...
    <div className="min-h-screen text-gray-100">
      ...
      <div className="container mx-auto p-4">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">RSS Feed Reader</h1>
          <p className="text-gray-400">Stay updated with your favorite content</p>
        </header>
            ...
            <div className="bg-gray-900 rounded-xl p-5 shadow-lg mb-6">
              ...
              <div className="mb-6">
                ...
                <button
                  ...
                  className={`w-full bg-blue-600 text-white font-medium rounded-lg py-3 px-4 flex items-center justify-center transition-all duration-200 ${processingStatus.isProcessing || !feedUrl.trim()
              ...
              <div className="mt-4">
                ...
                {loadingFeeds ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map(i => (
                      <div key={i} className="bg-gray-800/50 rounded-lg p-3 animate-pulse h-14"></div>
                    ))}
                  </div>
                ) : feeds.length ? (
                  <div className="space-y-2 max-h-96 overflow-y-auto pr-1 custom-scrollbar">
                    {feeds.map((feed) => (
                      ...
                      >
                        ...
                        >
                          <Rss className={`h-4 w-4 mr-3 flex-shrink-0 ${selectedFeed === feed.feed_url
                            ? "text-blue-400"
                        ...
                        <button
                          onClick={() => viewFeedDetails(feed.id, feed.feed_url)}
          ...
          <div className="lg:col-span-3">
            {selectedFeed ? (
              <div className="bg-gray-900 rounded-xl p-5 shadow-lg min-h-[400px]">
                ...
                {loadingItems ? (
                        ...
                        <article className="p-5 rounded-xl bg-gray-800/40 border border-gray-700/50 hover:border-gray-700 transition-all duration-200">
                          ...
                          {(item.description || item.content) && (
                            <div className="mt-3">
                              ...
                              >
                                {showDescription[idx] ? "Hide description" : "Show description"}
                              </button>
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="text-gray-500">This feed doesn't contain any articles or may be invalid</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="bg-gray-900 rounded-xl p-8 shadow-lg text-center py-20">
                ...
                <h2 className="text-2xl font-medium text-gray-300 mb-3">No feed selected</h2>
                <p className="text-gray-500 max-w-md mx-auto mb-6">
                  Select a feed from the sidebar or add a new RSS feed to get started
                </p>
                {feeds.length > 0 && (
                  <div className="max-w-xs mx-auto">
                    <div className="text-sm text-left mb-2 text-gray-400">Quick select:</div>
            ...
            <div className="bg-gray-900 rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
              ...
              <div className="p-5 overflow-y-auto flex-1">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-white break-all">{selectedFeedDetails.url}</p>
                </div>

                <div>
                  <h4 className="text-gray-400 text-sm mb-3">Feed Content URLs</h4>
                  <div className="bg-gray-800 rounded-lg p-4 space-y-3">
                    {loadingItems ? (
                      ...
                      <ul className="space-y-2">
                        {feedItems.map((item, idx) => (
                          <li key={idx} className="border-b border-gray-700 pb-2 last:border-0 last:pb-0">
                            <a
                              href={item.link}
                              target="_blank"
                              rel="noopener noreferrer"
                            ...
                            >
                              ...
                              <span className="flex-1">{item.title || item.link}</span>
                      ...
                      <p className="text-gray-500 text-sm">No URLs found in this feed or feed not loaded.</p>
                    )}
                  </div>
                </div>
              </div>
